...
$sql = "SELECT KodeTanaman, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganTanaman, JenisTanaman, LuasTanam, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_tanaman {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeTanaman, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganTanaman, JenisTanaman, LuasTanam, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_tanaman WHERE {$dw2a}";
    $sql.=" KodeTanaman LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganTanaman LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  JenisTanaman LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    
    $sql.=" OR  LuasTanam LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeTanaman"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganTanaman"]; 
    $nestedData[] = $row["JenisTanaman"]; 
    $nestedData[] = $row["LuasTanam"];
