...
$sql = "SELECT KodeBuku, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBuku, JenisBuku, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_buku {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeBuku, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBuku, JenisBuku, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_buku WHERE {$dw2a}";
    $sql.=" KodeBuku LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganBuku LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  JenisBuku LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    ...
    $nestedData[] = $row["GolonganBuku"]; 
    $nestedData[] = $row["JenisBuku"]; 
