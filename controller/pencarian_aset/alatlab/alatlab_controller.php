...
$sql = "SELECT KodeAlatLab, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatLab, NamaBarangLaboratorium, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatlab {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatLab , KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatLab, NamaBarangLaboratorium, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatlab WHERE {$dw2a}";
    $sql.=" KodeAlatLab LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatLab LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangLaboratorium LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatLab"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatLab"]; 
    $nestedData[] = $row["NamaBarangLaboratorium"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
