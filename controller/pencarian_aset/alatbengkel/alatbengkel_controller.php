...
$sql = "SELECT KodeAlatBengkel, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatBengkel, NamaBarangBengkel, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatbengkel {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatBengkel, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatBengkel, NamaBarangBengkel, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatbengkel WHERE {$dw2a}";
    $sql.=" KodeAlatBengkel LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatBengkel LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangBengkel LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatBengkel"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatBengkel"]; 
    $nestedData[] = $row["NamaBarangBengkel"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
