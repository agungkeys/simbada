...
$sql = "SELECT KodeAlatAngkutan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatAngkutan, NamaAlatAngkutan, Merk, Tipe, Kapasitas, Warna, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatangkutan {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatAngkutan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatAngkutan, NamaAlatAngkutan, Merk, Tipe, Kapasitas, Warna, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatangkutan WHERE {$dw2a}";
    $sql.=" KodeAlatAngkutan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatAngkutan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaAlatAngkutan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  Warna LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql = "SELECT KodeAlatAngkutan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatAngkutan, NamaAlatAngkutan, Merk, Tipe, Kapasitas, Warna, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatangkutan {$dw3}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatAngkutan"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatAngkutan"]; 
    $nestedData[] = $row["NamaAlatAngkutan"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
