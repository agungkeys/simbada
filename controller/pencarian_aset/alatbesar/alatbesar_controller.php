...
$sql = "SELECT KodeAlatBesar, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatBesar, NamaAlatBesar, Merk, Tipe, Kapasitas, Model, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatbesar {$dw1}";


$query=mysqli_query($conn, $sql) or die("master_aset_alatbesar_controller: Get DataAlat Besar");
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatBesar, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatBesar, NamaAlatBesar, Merk, Tipe, Kapasitas, Model, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatbesar WHERE {$dw2a}";
    $sql.=" KodeAlatBesar LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatBesar LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaAlatBesar LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  Merk LIKE '%".$requestData['search']['value']."%' {$dw2b}";

    $sql.=" OR  Tipe LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  Kapasitas LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  Model LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NomorRangka LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    
    $sql.=" OR  NomorMesin LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $query=mysqli_query($conn, $sql) or die("master_aset_alatbesar_controller: Get AsetAlat Besar from Search 2"); // again run query with limit
    // $num_rows = mysql_num_rows($query);
    
} else {    
    $sql = "SELECT KodeAlatBesar, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatBesar, NamaAlatBesar, Merk, Tipe, Kapasitas, Model, NomorRangka, NomorMesin, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatbesar {$dw3}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatBesar"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatBesar"]; 
    $nestedData[] = $row["NamaAlatBesar"]; 
    // $nestedData[] = $row["Letak"];
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
    $nestedData[] = $row["Kapasitas"];
    $nestedData[] = $row["Model"];
    $nestedData[] = $row["NomorRangka"];
    $nestedData[] = $row["NomorMesin"]; 
