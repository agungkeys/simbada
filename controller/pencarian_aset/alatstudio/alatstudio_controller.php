...
$sql = "SELECT KodeAlatStudio, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatStudio, NamaBarangStudio, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatstudio {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatStudio, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatStudio, NamaBarangStudio, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatstudio WHERE {$dw2a}";
    $sql.=" KodeAlatStudio LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatStudio LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangStudio LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatStudio"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatStudio"]; 
    $nestedData[] = $row["NamaBarangStudio"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
