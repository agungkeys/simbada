...
$sql = "SELECT KodeBangunanAir, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanAir, NamaBangunanAir, Letak, Konstruksi, Bahan, Panjang, Lebar, Tinggi, FasilitasPenunjang,TahunPembuatan, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_air {$dw1}";


$query=mysqli_query($conn, $sql) or die("master_aset_air_controller: Get Data Air");
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeBangunanAir, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanAir, NamaBangunanAir, Letak, Konstruksi, Bahan, Panjang, Lebar, Tinggi, FasilitasPenunjang, TahunPembuatan, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_air WHERE {$dw2a}";
    $sql.=" KodeBangunanAir LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganBangunanAir LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBangunanAir LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $query=mysqli_query($conn, $sql) or die("master_aset_air_controller: Get Aset Air from Search 2"); // again run query with limit
    // $num_rows = mysql_num_rows($query);
    
} else {    
    $sql = "SELECT KodeBangunanAir, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanAir, NamaBangunanAir, Letak, Konstruksi, Bahan, Panjang, Lebar, Tinggi, FasilitasPenunjang,TahunPembuatan, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_air {$dw3}";
    $sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."   LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
    $query=mysqli_query($conn, $sql) or die("master_aset_air_controller: Get Aset Jembatan from Search false");
    // $num_rows = mysql_num_rows($query);
}
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeBangunanAir"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganBangunanAir"]; 
    $nestedData[] = $row["NamaBangunanAir"]; 
