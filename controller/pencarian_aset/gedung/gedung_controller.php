...
// datatable column index  => database column name
    0 => 'KodeBangunanGedung', 
    1 => 'SubUnit',
    2 => 'SatuanKerja',
    3 => 'NamaPemilik', 
    4 => 'GolonganBangunanGedung', 
    5 => 'NamaBangunan', 
    6 => 'Letak',
    7 => 'LuasTanah',
    8 => 'LuasBangunan',
...
$sql = "SELECT KodeBangunanGedung, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanGedung, NamaBangunan, Letak, LuasTanah, LuasBangunan, Konstruksi, TanggalDokumen, Tingkat, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_bangunangedung {$dw1}";


$query=mysqli_query($conn, $sql) or die("databangunangedung_controller: Get Data Bangunan Gedung");
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeBangunanGedung, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanGedung, NamaBangunan, Letak, LuasTanah, LuasBangunan, Konstruksi, TanggalDokumen, Tingkat, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_bangunangedung WHERE {$dw2a}";
    $sql.=" KodeBangunanGedung LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganBangunanGedung LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $query=mysqli_query($conn, $sql) or die("databangunangedung_controller: Get Aset Bangunan Gedung from Search 1");
    $totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result without limit in the query 

    $sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']." LIMIT ".$requestData['start']." ,".$requestData['length']."   "; // $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc , $requestData['start'] contains start row number ,$requestData['length'] contains limit length.
    $query=mysqli_query($conn, $sql) or die("databangunangedung_controller: Get Aset Bangunan Gedung from Search 2"); // again run query with limit
    // $num_rows = mysql_num_rows($query);
    
} else {    
    $sql = "SELECT KodeBangunanGedung, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBangunanGedung, NamaBangunan, Letak, LuasTanah, LuasBangunan, Konstruksi, TanggalDokumen, Tingkat, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_bangunangedung {$dw3}";
    $sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."   LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
    $query=mysqli_query($conn, $sql) or die("databangunangedung_controller: Get Aset Jembatan from Search false");
    // $num_rows = mysql_num_rows($query);
}
