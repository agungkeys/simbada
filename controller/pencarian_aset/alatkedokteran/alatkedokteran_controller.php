...
// datatable column index  => database column name
    0 => 'KodeAlatKedokteran', 
    1 => 'SubUnit',
    2 => 'SatuanKerja',
    3 => 'NamaPemilik', 
    4 => 'GolonganAlatKedokteran', 
    5 => 'NamaBarangKedokteran', 
...
$sql = "SELECT KodeAlatKedokteran, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKedokteran, NamaBarangKedokteran, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatkedokteran {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatKedokteran, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKedokteran, NamaBarangKedokteran, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkedokteran WHERE {$dw2a}";
    $sql.=" KodeAlatKedokteran LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatKedokteran LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangKedokteran LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql = "SELECT KodeAlatKedokteran, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKedokteran, NamaBarangKedokteran, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkedokteran {$dw3}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatKedokteran"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatKedokteran"]; 
    $nestedData[] = $row["NamaBarangKedokteran"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
