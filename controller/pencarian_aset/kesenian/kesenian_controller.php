...
// datatable column index  => database column name
    0 => 'KodeBarangKesenian', 
    1 => 'SubUnit',
    2 => 'SatuanKerja',
    3 => 'NamaPemilik', 
    4 => 'GolonganBarangKesenian', 
    5 => 'NamaBarangKesenian', 
...
$sql = "SELECT KodeBarangKesenian, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBarangKesenian, NamaBarangKesenian, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatkesenian {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeBarangKesenian, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBarangKesenian, NamaBarangKesenian, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkesenian WHERE {$dw2a}";
    $sql.=" KodeBarangKesenian LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganBarangKesenian LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangKesenian LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql = "SELECT KodeBarangKesenian, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganBarangKesenian, NamaBarangKesenian, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkesenian {$dw3}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeBarangKesenian"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganBarangKesenian"]; 
    $nestedData[] = $row["NamaBarangKesenian"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
