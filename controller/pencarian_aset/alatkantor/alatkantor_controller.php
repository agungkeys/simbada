...
$sql = "SELECT KodeAlatKantor, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKantor, NamaBarangKantor, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatkantor {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatKantor, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKantor, NamaBarangKantor, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkantor WHERE {$dw2a}";
    $sql.=" KodeAlatKantor LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatKantor LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangKantor LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatKantor"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatKantor"]; 
    $nestedData[] = $row["NamaBarangKantor"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
