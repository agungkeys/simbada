...
$sql = "SELECT KodeHewan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganHewan, JenisHewan, JenisKelamin, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_hewan {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeHewan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganHewan, JenisHewan, JenisKelamin, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_hewan WHERE {$dw2a}";
    $sql.=" KodeHewan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganHewan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  JenisHewan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  JenisKelamin LIKE '%".$requestData['search']['value']."%' {$dw2b}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeHewan"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganHewan"]; 
    $nestedData[] = $row["JenisHewan"]; 
    $nestedData[] = $row["JenisKelamin"];
