...
// datatable column index  => database column name
    ...
    2 => 'SatuanKerja',
    3 => 'NamaPemilik', 
    4 => 'GolonganAlatKeamanan', 
    5 => 'NamaBarangKeamanan', 
...
$sql = "SELECT KodeAlatKeamanan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKeamanan, NamaBarangKeamanan, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
$sql.=" FROM aset_alatkeamanan {$dw1}";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT KodeAlatKeamanan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKeamanan, NamaBarangKeamanan, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkeamanan WHERE {$dw2a}";
    $sql.=" KodeAlatKeamanan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql.=" OR  GolonganAlatKeamanan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    $sql.=" OR  NamaBarangKeamanan LIKE '%".$requestData['search']['value']."%' {$dw2b}";
    ...
    $sql = "SELECT KodeAlatKeamanan, KodeLokasi, SubUnit, SatuanKerja, NamaPemilik, GolonganAlatKeamanan, NamaBarangKeamanan, Merk, Tipe, Bahan, Ukuran, Jumlah, TahunPerolehan, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, EntryUser, Status";
    $sql.=" FROM aset_alatkeamanan {$dw3}";
...
while( $row=mysqli_fetch_array($query)) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $row["KodeAlatKeamanan"]; 
    $nestedData[] = $row["SubUnit"];
    $nestedData[] = $row["SatuanKerja"];
    $nestedData[] = $row["NamaPemilik"]; 
    $nestedData[] = $row["GolonganAlatKeamanan"]; 
    $nestedData[] = $row["NamaBarangKeamanan"]; 
    $nestedData[] = $row["Merk"];
    $nestedData[] = $row["Tipe"];
