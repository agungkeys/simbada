<?php
	...
	$sq = "SELECT KodeHewan FROM datahewan"; 
	$ress = $mysqli->query($sq);

	$json = [];
	while($row = $ress->fetch_assoc()){
	     $json[] = ['kode'=>substr($row['KodeHewan'], 2, 8)];
	}
	...
	$post = $_POST;
	$sql = "INSERT INTO datahewan (KodeHewan, KodeBarang, KodeLokasi, GolonganHewan, JenisHewan, TahunPerolehan, JenisKelamin, Jumlah, Kondisi, AsalUsul, NilaiPerolehan, Keterangan, PenanggungJawab, LokasiPenanggungJawab, Surveyor,  TglSurvey, MataUang, SatuanKerja, KodePemilik, NoReg, EntryUser)
	VALUES (
		'".$resid."',
		'".$post['1']."',
		'".$post['2']."',
	...
	$sql = "SELECT * FROM datahewan WHERE KodeHewan = '".$resid."'"; 
	$result = $mysqli->query($sql);
