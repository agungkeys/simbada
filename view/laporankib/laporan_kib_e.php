...
class MYPDF extends TCPDF
{
  ...
  function ImprovedTable($header, $data, $data1, $datasignature){
    ...
    if ($tott != 0)  $this->MultiCell(57, 7,'Rp '.number_format($tott, 2, ",", ".") , 1, 'L', 1, 0, '', '', true);
...
// Tampung Filter
  ...
  while($row = $result->fetch_assoc()){
    ...
    $json[] = [$no, $row["NamaJenisBarang"]." / ".$row["NamaBarang"], $row["KodeBarang"], $row["NoReg"], $row["JenisBuku"], $row["GolonganBuku"], $row["AsalDaerah"], $row["Pencipta"], $row["Bahan"], $row["JenisHewan"], $row["Ukuran"], $row["Jumlah"], $row["TahunPerolehan"], 'Rp '.number_format($row["NilaiPerolehan"], 2, ",", "."), $row["Keterangan"]];
