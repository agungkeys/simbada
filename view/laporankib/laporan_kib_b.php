...
class MYPDF extends TCPDF
{
  ...
  function ImprovedTable($header, $data, $data1, $datasignature){
    ...
    if ($tott != 0)  $this->MultiCell(59.9, 7,number_format($tott, 0, "", ".") , 1, 'L', 1, 0, '', '', true);
...
$sql = "SELECT view_kibb.KodeLokasi, masterbarang.NamaBarang, view_kibb.NoReg, view_kibb.Merk, view_kibb.Tipe, view_kibb.Kapasitas, view_kibb.Bahan, view_kibb.TahunPembelian, view_kibb.NomorPabrik, view_kibb.NomorRangka, view_kibb.NomorMesin, view_kibb.NomorPolisi, view_kibb.NomorBPKB, view_kibb.AsalUsul, view_kibb.NilaiPerolehan, view_kibb.Keterangan FROM view_kibb INNER JOIN masterbarang ON view_kibb.KodeBarang = masterbarang.KodeBarang WHERE KodeLokasi IN({$data_arr_location}) {$valasalusul} {$valtahunbetween}"; 
...
while($row = $result->fetch_assoc()){
  ...
  $json[] = [$no, $row["NamaBarang"], $row["NoReg"], $row["Merk"]." / ".$row["Tipe"], $row["Kapasitas"], $row["Bahan"], $row["TahunPembelian"], $row["NomorPabrik"], $row["NomorRangka"], $row["NomorMesin"], $row["NomorPolisi"], $row["NomorBPKB"], $row["AsalUsul"], number_format($row["NilaiPerolehan"], 0, "", "."), $row["Keterangan"]];
...
$pdf->SetFont('Times', '', 11);
$pdf->MultiCell(93, 5, 'KODE BARANG: 02', 0, 'R', 0, 1, '', '', true);
