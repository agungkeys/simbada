...
class MYPDF extends TCPDF
{
  ...
  function ImprovedTable($header, $data, $data1, $datasignature){
    ...
    if ($tott != 0)  $this->MultiCell(63, 7, number_format($tott, 0, "", ".") , 1, 'L', 1, 0, '', '', true);
...
while($row = $result->fetch_assoc()){
  ...
  $json[] = [$no, $row["NamaBarang"], $row["KodeBarang"], $row["NoReg"], number_format($row["LuasTanah"], 0, "", "."), $row["TahunPerolehan"], $row["Letak"], $row["StatusTanah"], date("d/m/Y", strtotime($row["Tanggal"])), $row["Nomor"], $row["Penggunaan"], $row["AsalUsul"],number_format($row["NilaiPerolehan"], 0, "", "."), $row["Keterangan"]];
