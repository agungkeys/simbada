...
$pdf->SetTitle('Laporan Extra Countable (Kapitalisasi)');
$pdf->SetSubject('Laporan Extra Countable (Kapitalisasi)');
$pdf->SetKeywords('Laporan Extra Countable (Kapitalisasi)');
...
$sql = "SELECT KodeLokasi, KodeBarang, NoReg, JenisNamaBarang, NamaBarang, Merk, NomorPabrik, Bahan, AsalUsul, TahunPembelian, Ukuran, Satuan, Kondisi, Jumlah, Nilai, KetStatus FROM view_invallhapus WHERE KodeLokasi IN({$data_arr_location}) {$valasalusul} {$valtahunbetween}  AND Status IN('XXXXXXX') "; 
...
$pdf->MultiCell(93, 5, '', 0, 'L', 0, 0, '', '', true);
$pdf->MultiCell(92, 5, 'EXTRA COUNTABLE (KAPITALISASI)', 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(93, 5, '', 0, 'R', 0, 1, '', '', true);
